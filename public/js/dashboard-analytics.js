...
async function fetchAnalyticsData(force = false) {
  ...
  try {
    const json = await (await authFetch('/api/analytics-router.php?action=analytics&' + params)).json();
...
async function fetchExtendedAnalytics() {
  ...
  const params=new URLSearchParams({range:anFilters.range});
  if (anFilters.region) params.set('region',anFilters.region);
  try { const json=await(await authFetch('/api/analytics-router.php?action=analytics-extended&'+params)).json(); if(json.success){AN_EXT=json;AN_EXT_CACHE_KEY=cacheKey;} } catch(e) {}
}
...
async function admLoadDisabilityDist() {
  try {
    const json=await(await authFetch('/api/analytics-router.php?action=regions-stats')).json();
...
async function admRenderDisBars() {
  ...
  if(region){try{const res=await(await authFetch('/api/analytics-router.php?action=regions-stats&region='+encodeURIComponent(region))).json();dis=(res.success&&res.disabilities)?res.disabilities:[];}catch(e){dis=[];}}
...
async function loadRegionsView() {
  if(!rgLoaded){
    ...
    try{const json=await(await authFetch('/api/analytics-router.php?action=regions-stats')).json();if(json.success){RG_DATA.summary=json.summary||{};RG_DATA.regions=json.regions||[];RG_DATA.provinces=json.provinces||[];RG_DATA.cities=json.cities||[];RG_DATA.disabilities=json.disabilities||[];RG_DATA.monthly_trend=json.monthly_trend||[];}}catch(e){}
    rgLoaded=true;
  }
  applyRgFilters();
  showRgTab('overview');
